...
namespace XMonitor_GUI
{
    public class SerialPortModel : SerialPort, INotifyPropertyChanged
    {
        ...
        /// <param name="Set_pensa_radius"></param>
        /// <param name="Set_work_height"></param>
        public void SendZhiBaoCanShu(float Set_UAV_ZhiBaoSpeed, float Set_UAV_ZhiBaoFuDu, float Set_UAV_ZhiBaoAction, float Set_UAV_ZhiBaoShuiBenEquipment, float Set_UAV_ZhiBaoShuiBenYaoLiang)
        {
            if (IsOpen == false)
            {
                MainWindow.pWin.open_com_flag = false;
            ...
            TxData[6] = (byte)(sTemp & 0x00FF);
            sTemp = (short)(Set_UAV_ZhiBaoShuiBenEquipment);
            TxData[7] = (byte)(sTemp & 0x00FF);
            sTemp = (short)(Set_UAV_ZhiBaoShuiBenYaoLiang);
            TxData[8] = (byte)(sTemp & 0x00FF);
            for (int i = 2; i <= 8; i++)
            {
                SBcc += (ushort)((ushort)TxData[i] & 0x00FF);
            }
            TxData[9] = (byte)((SBcc >> 8) & 0x00FF);
            TxData[10] = (byte)(SBcc & 0x00FF);
            try
            {
                Write(TxData, 0, 11);
            }
        ...
        /// <param name="Set_pensa_radius"></param>
        /// <param name="Set_work_height"></param>
        public void SendAnQuanCeLue(float Set_UAV_Noticefirstvolt, float Set_UAV_Noticesecondvolt, float Set_UAV_Noticesecondaction, float Set_UAV_Noticelosecontrolaction, float Set_UAV_Noticedistance, float Set_UAV_Noticeheight, float Set_UAV_Noticefanhanggaodu, float Set_UAV_NoticeIfObstacle)
        {
            if (IsOpen == false)
            {
                MainWindow.pWin.open_com_flag = false;
            ...
            sTemp = (short)(Set_UAV_NoticeIfObstacle);
